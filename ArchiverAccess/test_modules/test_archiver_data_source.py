...
class SQLAbstractionStub(object):
    ...
    def add_initial_value(
        ...
        archiver_data_value = ArchiverDataValue(
            [severity_id, status_id, num_val, float_val, str_val, array_val, sample_time]
        )
    ...
    def add_changes(
        ...
        archiver_data_value = ArchiverDataValue(
            [severity_id, status_id, num_val, float_val, str_val, array_val, smpl_time]
        )
        change = [channel_name]
...
class TestArchiverDataSource(unittest.TestCase):
    ...
    def test_GIVEN_no_results_for_pv_requested_WHEN_get_initial_values_THEN_None_value_returned(
        self,
    ):
        self.set_up_data_source()
    ...
    def test_GIVEN_too_many_results_for_pv_requested_WHEN_get_initial_values_THEN_exception_thrown(
        self,
    ):
        self.set_up_data_source()

        self.mysql_abstraction_layer.add_initial_value()
        self.mysql_abstraction_layer.initial_values[0].append(
            self.mysql_abstraction_layer.initial_values[0]
        )
    ...
    def test_GIVEN_results_for_multiple_pvs_requested_WHEN_get_initial_values_THEN_values_returned(
        self,
    ):
        self.set_up_data_source()
        expected_value1 = 2.2
        ...
        result = self._data_source.initial_values(
            ["pv one", "pv name two"], datetime(2017, 1, 2, 3, 4, 5)
        )

        assert_that(result, is_([expected_value1, expected_value2]))

    def test_GIVEN_single_integer_pv_requested_WHEN_get_changes_generator_values_THEN_value_returned(
        self,
    ):
        ...
        self.mysql_abstraction_layer.add_changes(
            smpl_time=expected_time_stamp, float_val=expected_value, channel_name=channel_name
        )
        ...
        for change in self._data_source.changes_generator(
            ...
            ArchiveTimePeriod(datetime(2017, 1, 2, 3, 4, 5), timedelta(seconds=1), 10),
        ):
            changes.append(change)

        assert_that(changes, is_([(expected_time_stamp, 0, expected_value)]))

    def test_GIVEN_multiple_pvs_requested_WHEN_get_changes_generator_values_THEN_values_returned(
        self,
    ):
        channel_name = "channel name"
        ...
        self.set_up_data_source()
        self.mysql_abstraction_layer.add_changes(
            smpl_time=expected_time_stamp, str_val=expected_value, channel_name=channel_name
        )
        self.mysql_abstraction_layer.add_changes(
            smpl_time=expected_time_stamp2, num_val=expected_value2, channel_name=channel_name2
        )
        ...
        assert_that(
            ...
            is_(
                [
                    (expected_time_stamp, 0, expected_value),
                    (expected_time_stamp2, 1, expected_value2),
                ]
            ),
        )

    def test_GIVEN_no_results_for_multiple_pvs_requested_WHEN_get_changes_THEN_blank_list_returned(
        self,
    ):
        channel_name = "channel name"
        channel_name2 = "channel name2"
    ...
    def test_GIVEN_query_gives_error_WHEN_get_changes_generator_values_THEN_error(self):
        ...
        self.mysql_abstraction_layer.query_returning_cursor = Mock(
            side_effect=DatabaseError("Problems with accessing the database")
        )

        gen = self._data_source.changes_generator(
            [
                channel_name,
            ],
            ArchiveTimePeriod(datetime(2017, 1, 2, 3, 4, 5), timedelta(seconds=1), 10),
    ...
    def test_GIVEN_nothing_WHEN_get_latest_sample_time_THEN_latest_sample_id_returned(self):
        ...
        assert_that(
            self.mysql_abstraction_layer.querry_parms[0],
            is_((expected_sample_id_from, expected_sample_id_from)),
        )
    ...
    def test_GIVEN_time_WHEN_get_latest_sample_time_THEN_latest_sample_time_returned(self):
        ...
        result_time, result_id = self._data_source.get_latest_sample_time(
            expected_sample_id_from, expected_datetime
        )
        ...
        assert_that(
            self.mysql_abstraction_layer.querry_parms[0],
            is_((expected_sample_id_from, expected_sample_id_from, expected_datetime)),
        )
    ...
    def test_GIVEN_single_integer_pv_requested_WHEN_get_changes_for_logging_THEN_value_returned(
        self,
    ):
        channel_name = "channel name"
        self.set_up_data_source()
        ...
        changes = []
        for change in self._data_source.logging_changes_for_sample_id_generator(
            [
                channel_name,
            ],
            datetime(2010, 9, 8, 2, 3, 4),
            datetime(2010, 9, 8, 2, 3, 34),
        ):
            changes.append(change)
    ...
    def test_GIVEN_no_pvs_requested_WHEN_get_changes_for_logging_THEN_no_values_returned(self):
        ...
        changes = []
        for change in self._data_source.logging_changes_for_sample_id_generator(
            [], datetime(2010, 9, 8, 2, 3, 4), datetime(2010, 9, 8, 2, 3, 34)
        ):
            changes.append(change)
