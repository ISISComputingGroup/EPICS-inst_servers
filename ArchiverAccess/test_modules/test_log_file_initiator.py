...
class TestLogFileInitiatorForContinousLogging(unittest.TestCase):
    def test_GIVEN_config_with_pv_WHEN_logging_pv_has_changed_0_to_1_THEN_log_file_created(self):
        ...
        archive_data_source = DataSourceMother.set_up_data_source(
            initial_pv_values=[0],
            data_changes=data_changes,
            logging_start_times=[datetime(2017, 1, 1, 1, 1, 1)],
        )
        write_file_header_mock = Mock()
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source, write_file_header_mock=write_file_header_mock
        )
    ...
    def test_GIVEN_config_with_pv_WHEN_logging_pv_has_swicthed_off_in_changes_THEN_log_file_body_written_and_file_made_readonly(
        self,
    ):
        ...
        data_changes = [[(expected_logging_start, 0, 1), (expected_logging_stop, 0, 0)]]
        archive_data_source = DataSourceMother.set_up_data_source(
            initial_pv_values=[0],
            data_changes=data_changes,
            logging_start_times=[datetime(2017, 1, 1, 1, 1, 1)],
        ...
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source,
            write_data_lines_mock=write_data_lines_mock,
            finish_log_file_mock=finished_log_file_mock,
        )

        log_file_initiator.check_initiated()

        write_data_lines_mock.assert_called_once_with(
            ArchiveTimePeriod(
                expected_logging_start, expected_period, finish_time=expected_logging_stop
            )
        )
        finished_log_file_mock.assert_called_once()

    def test_GIVEN_config_with_pv_WHEN_logging_switches_on_in_changes_period_THEN_log_file_body_written_to_end_of_period_finish_not_written(
        self,
    ):
        log_period_in_second = 1
        expected_period = timedelta(seconds=log_period_in_second)
        ...
        archive_data_source = DataSourceMother.set_up_data_source(
            ...
            data_changes=data_changes,
            logging_start_times=[datetime(2017, 1, 1, 1, 1, 1)],
            sample_times=[expected_end_time],
        )
        write_data_lines_mock = Mock()
        finished_log_file_mock = Mock()
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
        ...
        write_data_lines_mock.assert_called_once_with(
            ArchiveTimePeriod(
                expected_logging_start, expected_period, finish_time=expected_end_time
            )
        )
        finished_log_file_mock.assert_not_called()

    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_twice_from_1_to_0_THEN_two_continual_log_files_created(
        self,
    ):
        log_period_in_second = 1
        expected_period = timedelta(seconds=log_period_in_second)
        ...
        archive_data_source = DataSourceMother.set_up_data_source(
            initial_pv_values=[1],
            data_changes=data_changes,
            logging_start_times=[expected_logging_start1],
        )
        ...
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source,
            log_period_in_seconds=[log_period_in_second],
            write_data_lines_mock=write_data_lines_mock,
        )
        ...
        self.assert_logging_period_correct(
            logging_time_period1,
            expected_logging_start1,
            expected_logging_stop_time1,
            expected_period,
        )

        logging_time_period2 = arg_list[1][0][0]
        self.assert_logging_period_correct(
            logging_time_period2,
            expected_logging_start_time2,
            expected_logging_stop_time2,
            expected_period,
        )

    def assert_logging_period_correct(
        self,
        logging_time_period,
        expected_logging_start,
        expected_logging_stop_time,
        expected_period,
    ):
    ...
    def test_GIVEN_config_with_pv_WHEN_logging_pv_is_initially_at_1_THEN_log_file_header_created(
        self,
    ):
        expected_logging_start = datetime(2017, 1, 1, 1, 1, 2)
        initial_values = [1]
        archive_data_source = DataSourceMother.set_up_data_source(
            initial_pv_values=initial_values, logging_start_times=[expected_logging_start]
        )
        write_file_header_mock = Mock()
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source, write_file_header_mock=write_file_header_mock
...
class TestLogFileInitiator(unittest.TestCase):
    def test_GIVEN_config_with_pv_WHEN_get_data_THEN_correct_sample_times_asked_for(self):
        ...
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source,
            time_last_actived=time_last_active,
            sample_id_last_active=sample_id_last_active,
        )
    ...
    def test_GIVEN_config_with_pv_WHEN_check_write_twice_THEN_consecutive_sample_times_are_used(
        self,
    ):
        sample_ids = [90, 91]
        sample_id_last_active = 10
        ...
        archive_data_source = DataSourceMother.set_up_data_source(
            sample_times=sample_times, data_changes=[[], []], sample_ids=sample_ids
        )
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source,
            time_last_actived=time_last_active,
    ...
    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_from_1_to_0_THEN_log_file_created(self):
        ...
        archive_data_source = DataSourceMother.set_up_data_source(
            initial_pv_values=[1], final_pv_value=0
        )
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source, log_period_in_seconds=[log_period_in_second]
        )
    ...
    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_0_to_X_THEN_log_file_not_created(self):
        archive_data_source = DataSourceMother.set_up_data_source(
            initial_pv_values=[0], final_pv_value=1
        )
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source
        )
    ...
    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_0_to_0_THEN_log_file_not_created(self):
        archive_data_source = DataSourceMother.set_up_data_source(
            initial_pv_values=[0], final_pv_value=0
        )
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source
        )
    ...
    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_1_to_disconnect_THEN_log_file_created(self):
        archive_data_source = DataSourceMother.set_up_data_source(
            initial_pv_values=[1], final_pv_value="disconnect"
        )
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source
        )
    ...
    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_disconnected_to_0_THEN_log_file_not_created(
        self,
    ):
        archive_data_source = DataSourceMother.set_up_data_source(
            initial_pv_values=["disconnect"], final_pv_value=1
        )
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source
        )
    ...
    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_twice_from_1_to_0_over_two_different_write_checks_THEN_two_log_files_created(
        ...
        sample_times = [
            ...
            datetime(2001, 2, 3, 4, 5, 36),
            datetime(2001, 2, 3, 4, 6, 6),
        ]
        data_changes = [
            [],
            [
    ...
    def test_GIVEN_multiple_configs_with_pv_WHEN_pvs_has_changed_from_1_to_0_THEN_multiple_log_files_created(
        self,
    ):
        ...
        data_changes = [
            [(datetime(2017, 1, 1, 1, 1, 2), 0, 0), (datetime(2017, 1, 1, 1, 2, 2), 1, 0)]
        ]

        archive_data_source = DataSourceMother.set_up_data_source(
            initial_pv_values=[1, 1],
            data_changes=data_changes,
            logging_start_times=[expected_logging_start_config_1, expected_logging_start_config_2],
        )
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source,
            log_period_in_seconds=[log_period_in_second1, log_period_in_second2],
        )
    ...
    def test_GIVEN_config_with_logging_period_which_is_a_pv_WHEN_log_THEN_logging_period_is_pv_value(
        self,
    ):
        ...
        archive_data_source = DataSourceMother.set_up_data_source(
            initial_pv_values=[1],
            final_pv_value=0,
            logging_period_pv_values=logging_period_pv_values,
        )
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source, log_period_pvs=[pv_name]
        )
    ...
    def test_GIVEN_config_with_pv_WHEN_pv_has_changed_from_1_to_0_and_write_file_throws_THEN_no_error_thrown_log_not_written(
        self,
    ):
        log_period_in_second = 1
        archive_data_source = DataSourceMother.set_up_data_source(
        ...
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source,
            log_period_in_seconds=[log_period_in_second],
            throw_on_write_complete_file=True,
        )
        ...
        try:
            ...
            self.fail(
                "If underlying call throws then this should catch and logs. Error: '{}'".format(e)
            )

        self.log_file_creators[0].write_complete_file.assert_called_once()

    def test_GIVEN_config_with_pv_WHEN_get_data__and_no_new_sample_time_THEN_sample_time_is_current_time_minus_set_amount(
        self,
    ):
        ...
        archive_data_source = DataSourceMother.set_up_data_source(sample_times=sample_times)
        log_file_initiator, self.log_file_creators = DataSourceMother.create_log_file_intiator(
            archive_data_source, current_time=current_time
        )
...
class DataSourceMother(object):
    ...
    def set_up_data_source(
        ...
        archive_data_source = ArchiverDataStub(
            initial_archiver_data_value=initial_archiver_data_values,
            data_changes=data_changes,
            sample_times=sample_times,
    ...
    @staticmethod
    def create_log_file_intiator(
        archive_data_source,
        log_period_in_seconds=None,
        log_period_pvs=None,
        throw_on_write_complete_file=False,
        write_file_header_mock=Mock(),
        write_data_lines_mock=Mock(),
        finish_log_file_mock=Mock(),
        current_time=datetime(2000, 1, 1, 1, 1, 2),
        time_last_actived=datetime(2000, 1, 1, 1, 1, 1),
        ...
        for log_period_in_second, log_period_pv in zip(log_period_in_seconds, log_period_pvs):
            ...
            if throw_on_write_complete_file:
                log_file_creator.write_complete_file = Mock(
                    side_effect=DataFileCreationError("Test problem")
                )
            else:
                log_file_creator.write_complete_file = Mock()
        ...
        return LogFileInitiatorOnPVChange(
            configs,
            archive_data_source,
            time_last_active,
            get_current_time,
