...
class TestDatabaseConfigBuilder(TestCase):
    def test_GIVEN_ioc_name_WHEN_generate_THEN_configuration_with_correct_file_name_template_is_created(
        self,
    ):
        ioc_name = "myioc"
        expected_filename_template = os.path.join(
            DEFAULT_LOG_PATH, "myioc", "myioc_{start_time}.dat"
        )
        expected_continous_filename_template = os.path.join(
            DEFAULT_LOG_PATH, "myioc", "myioc_{start_time}_continuous.dat"
        )
        ...
        assert_that(
            config[0].continuous_logging_filename_template,
            is_(expected_continous_filename_template),
        )
    ...
    def test_GIVEN_templated_header_line_in_database_WHEN_generate_THEN_configuration_is_created(
        self,
    ):
        pv_name = "pv:name"
        ...
        ioc_data_source = self._create_ioc_data_source(
            header_lines=[header_line], header_pvs=[pv_name]
        )
        db_config_builder = ArchiverAccessDatabaseConfigBuilder(ioc_data_source)
    ...
    def test_GIVEN_multiple_header_line_in_database_WHEN_generate_THEN_configuration_is_created(
        self,
    ):
    ...
    def test_GIVEN_multiple_header_line_in_funny_order_with_repeated_indexes_and_different_cases_WHEN_generate_THEN_configuration_is_created(
        self,
    ):
        expected_header_lines = ["line 1", "line2", "line3", "line4"]
        header_lines_section = [
            ("pvname", "LoG_header6", expected_header_lines[3]),
    ...
    def test_GIVEN_trigger_pv_only_marked_WHEN_generate_THEN_configuration_has_trigger_pv_in(self):
        ...
        ioc_data_source = self._create_ioc_data_source(
            trigger_pv=expected_trigger_pv, trigger_pv_template=""
        )
        db_config_builder = ArchiverAccessDatabaseConfigBuilder(ioc_data_source)
    ...
    def test_GIVEN_trigger_pv_is_explicit_WHEN_generate_THEN_configuration_has_trigger_pv_in(self):
        ...
        ioc_data_source = self._create_ioc_data_source(
            trigger_pv="blah", trigger_pv_template=trigger_pv
        )
        db_config_builder = ArchiverAccessDatabaseConfigBuilder(ioc_data_source)
    ...
    def test_GIVEN_logging_period_pv_is_only_marked_WHEN_generate_THEN_configuration_has_logging_period_pv(
        self,
    ):
        ...
        logging_period = config[0].logging_period_provider.get_logging_period(
            archive_data_source, None
        )
        assert_that(logging_period, is_(expected_log_period))

    def test_GIVEN_logging_period_pv_is_templated_WHEN_generate_THEN_configuration_has_logging_period_pv(
        self,
    ):
        period_pv = "diff:triggerpv"
        ioc_data_source = self._create_ioc_data_source(
            period_pv=period_pv, period_pv_template="this_pv"
        )
        db_config_builder = ArchiverAccessDatabaseConfigBuilder(ioc_data_source)
        log_period = 10
        expected_log_period = timedelta(seconds=log_period)
    ...
    def test_GIVEN_logging_period_pv_is_explicit_WHEN_generate_THEN_configuration_has_logging_period_pv(
        self,
    ):
        period_pv = "diff:triggerpv.field"
        ioc_data_source = self._create_ioc_data_source(
            period_pv="blah", period_pv_template=period_pv
        )
        db_config_builder = ArchiverAccessDatabaseConfigBuilder(ioc_data_source)
        log_period = 10
        expected_log_period = timedelta(seconds=log_period)
    ...
    def test_GIVEN_logging_period_is_a_constant_WHEN_generate_THEN_configuration_has_correct_logging_period(
        self,
    ):
    ...
    def test_GIVEN_logging_period_is_a_constant_which_is_invalid_WHEN_generate_THEN_configuration_has_default_logging_period(
        self,
    ):
    ...
    def test_GIVEN_column_with_only_header_in_database_WHEN_generate_THEN_configuration_has_assoicated_pv_and_header(
        self,
    ):
        ...
        expected_pv_name = pv_name + ".VAL"
        ioc_data_source = self._create_ioc_data_source(
            column_headers=[(pv_name, 1, expected_column_header)]
        )
        db_config_builder = ArchiverAccessDatabaseConfigBuilder(ioc_data_source)
    ...
    def test_GIVEN_column_with_template_value_no_header_in_database_WHEN_generate_THEN_configuration_has_correct_pv_and_header_is_template(
        self,
    ):
        pv_name = "pv:name"
        expected_pv_name = pv_name + ".VAL"
    ...
    def test_GIVEN_column_with_empty_value_no_header_in_database_WHEN_generate_THEN_configuration_has_correct_pv_and_header_is_pv_name(
        self,
    ):
        pv_name = "pv:name"
        expected_pv_name = pv_name + ".VAL"
    ...
    def test_GIVEN_column_with_template_value_and_header_in_database_WHEN_generate_THEN_configuration_has_correct_pv_and_header(
        self,
    ):
        pv_name = "pv:name"
        expected_pv_name = pv_name + ".VAL"
    ...
    def test_GIVEN_columns_with_multiple_template_values_and_header_some_with_both_some_with_one_in_database_WHEN_generate_THEN_configuration_has_correct_pv_and_header(
        self,
    ):
        expected_pv_names = [
            "pv:name0.val",
        ...
        assert_that(
            config[0]
            .table_line.format(*config[0].pv_names_in_columns, time="time")
            .split("\t")[1:],
            is_(expected_pv_names),
        )

    def _create_ioc_data_source(
        self,
        ioc_name="ioc",
        header_lines=None,
        header_pvs=None,
        header_lines_section=None,
        trigger_pv="trigger_pv",
        trigger_pv_template="this_pv",
        period_pv="period_pv",
        period_pv_template="this_pv",
        period_constant=None,
        column_headers=None,
