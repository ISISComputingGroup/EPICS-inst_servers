...
class TestTimeLastActive(TestCase):
    def test_GIVEN_there_is_time_last_active_file_WHEN_get_time_last_active_THEN_date_time_in_file_returned(
        self,
    ):
        expected_time = datetime(2016, 1, 2, 3, 4, 5)
        time_last_active = self._setup_last_active_time(
            expected_time, 1, expected_time + timedelta(seconds=10)
        )
    ...
    def test_GIVEN_there_is_time_last_active_file_WHEN_get_time_last_active_THEN_last_sample_in_file_returned(
        self,
    ):
        expected_sample_id = 123
        time = datetime(2016, 1, 2, 3, 4, 5)
        time_last_active = self._setup_last_active_time(
            time, 1, time + timedelta(seconds=10), expected_sample_id
        )
    ...
    def test_GIVEN_there_is_time_last_active_file_which_is_earlier_than_max_allowed_WHEN_get_time_last_active_THEN_current_date_minus_max_is_returned_sample_id_is_still_id_in_file(
        self,
    ):
        max_delta = 2
        ...
        expected_sample_id = 123
        time_last_active = self._setup_last_active_time(
            early_time, max_delta, time_now, expected_sample_id
        )
    ...
    def test_GIVEN_delta_is_not_a_number_WHEN_get_time_last_active_THEN_current_date_minus_default(
        self,
    ):
        max_delta = "hi"
        time_now = datetime(2016, 1, 6, 3, 4, 5)
    ...
    def test_GIVEN_last_time_is_invalid_WHEN_get_time_last_active_THEN_current_time_minus_default_delta(
        self,
    ):
        max_delta = 2
        time_now = datetime(2016, 1, 6, 3, 4, 5)
    ...
    def test_GIVEN_last_time_active_WHEN_set_time_last_active_THEN_file_contains_last_time_active(
        self,
    ):
        ...
        assert_that(
            FileStub.contents_of_only_file(),
            is_(
                [
                    TIME_LAST_ACTIVE_HEADER,
    ...
    def _setup_last_active_time(self, last_active_time, max_delta_in_days, time_now, sample_id=100):
        ...
        try:
            ...
            FileStub.add_file(
                [TIME_LAST_ACTIVE_HEADER, last_active_time, max_delta_in_days, sample_id],
                TIME_LAST_ACTIVE_FILENAME,
            )
