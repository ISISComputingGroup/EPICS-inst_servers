...
from BlockServer.fileIO import file_manager
...
class TestConfigMonitor(unittest.TestCase):
    ...
    def test_WHEN_start_monitoring_THEN_monitor_called(
        self, epicsmonitor, get_hostname_from_prefix_mock
    ):
        ...
        epicsmonitor.assert_called_with(
            "{}CS:BLOCKSERVER:GET_CURR_CONFIG_DETAILS".format(REMOTE_TEST_PREFIX)
        )
    ...
    @patch("RemoteIocServer.config_monitor.get_hostname_from_prefix")
    @patch("RemoteIocServer.config_monitor._EpicsMonitor")
    def test_WHEN_remote_pv_prefix_changed_THEN_new_monitor_created(
        self, epicsmonitor, get_hostname
    ):
        get_hostname.return_value = "localhost"
        monitor = ConfigurationMonitor(LOCAL_TEST_PREFIX, lambda *a, **k: None)
        monitor.set_remote_pv_prefix(REMOTE_TEST_PREFIX)
                        ...
                        "component_iocs": [
                            {
                                ...
                                "pvsets": [],
                                "component": "_base",
                                "remotePvPrefix": LOCAL_TEST_PREFIX,
                                "restart": True,
                                "simlevel": "none",
                            },
                            {
...
""".format(pf=LOCAL_TEST_PREFIX)
    ...
    def test_GIVEN_write_standard_config_files_called_THEN_standard_config_files_written(
        ...
        with (
            ...
            patch.object(file_manager.os.path, "isdir", return_value=True),
        ):
            monitor = ConfigurationMonitor(LOCAL_TEST_PREFIX, lambda *a, **k: None)
            monitor.write_new_config_as_xml("{}")
            ...
            all_writes = [
                args[0][0]
                for args in mock_open.return_value.__enter__.return_value.write.call_args_list
            ]
    ...
    def test_GIVEN_update_last_config_called_THEN_standard_config_files_written(
        ...
        with patch.object(
            FILEPATH_MANAGER, "get_last_config_file_path", return_value=expected_path
        ):
            monitor.write_new_config_as_xml("{}")
