...
def create_log(
    headers, columns, time_period, default_field, filename_template="default.log", host="127.0.0.1"
):
    """
...
if __name__ == "__main__":
    description = (
        "Create a log file from the archive. E.g. python ArchiverAccess\log_file_generator.py "
    ...
    parser.add_argument(
        "--point_count", "-c", type=int, help="Number of sample points", required=True
    )
    parser.add_argument(
        "--start_time",
        "-s",
    ...
    parser.add_argument(
        "--delta_time", "-d", type=float, help="The time between points in seconds", required=True
    )
    parser.add_argument(
    ...
    the_time_period = ArchiveTimePeriod(
        data_start_time, timedelta(seconds=args.delta_time), args.point_count
    )
