...
base = dict(
    name="Z_Stage", size=(1500.0, 1500.0, 2650.0), color=GREY
)  # Initial height is halfway across travel
...
incident_slits = dict(
    name="Incident_Slits", position=(-1200, 0, 0), size=(80.0, 300.0, 300.0), color=WHITE
)  # fixed for now

# Stationary objects
camera = dict(
    name="Camera", position=(800.0 + 440.0 / 2, 0, -392), size=(440.0, 440.0, 1245.0), color=WHITE
)
...
def moves(axes):
    ...
    v.matrix = np.dot(
        u.matrix, t.matrix
    )  # This may be wrong order or may not be needed if you use t as a starting point for u
...
pvs = [
    "{}MOT:MTR0502",
    "{}MOT:MTR0503",
    "{}MOT:MTR0504",  # Tomography Phi, Chi, Theta
    "{}MOT:MTR0505",
    "{}MOT:MTR0506",  # Sample Stack X, Y
    "{}MOT:MTR0901",
    "{}MOT:MTR0902",  # Beckhoff rot & z
]

pvs = [pv.format(pv_prefix) for pv in pvs]

hardlimits = [[-90, 270], [-6, 6], [-6, 6], [105, 1005], [105, 1005], [180, -180], [-250, 250]]
