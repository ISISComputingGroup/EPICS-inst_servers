...
class TestEpicsGateway(unittest.TestCase):
    def setUp(self):
        ...
        self.gateway = Gateway(
            self.gateway_prefix,
            self.prefix,
            self.gateway_file_path,
            self.block_prefix,
            self.control_sys_prefix,
        )

    @patch("BlockServer.epics.gateway.ChannelAccess")
    def test_GIVEN_gateway_pv_doesnt_exist_WHEN_exist_called_THEN_returns_false(
        self, channel_access
    ):
    ...
    def test_GIVEN_local_PV_without_suffix_or_SP_WHEN_alias_generated_THEN_lines_as_expected(self):
        ...
        expected_lines = [
            [r"{}\([.:].*\)".format(alias), "ALIAS", "INST:MY_PV\\1"],
    ...
    def test_GIVEN_local_PV_with_SP_WHEN_alias_generated_THEN_lines_as_expected(self):
        ...
        alias = "INST:BLOCK:MY_BLOCK"
        expected_lines = [
            [r"{}\(:SP\)?\([.:].*\)".format(alias), "ALIAS", "INST:MY_PV:SP\\2"],
    ...
    def test_GIVEN_remote_PV_with_SP_WHEN_alias_generated_THEN_lines_as_expected(self):
        ...
        alias = "INST:BLOCK:MY_BLOCK"
        expected_lines = [
            [r"{}\(:SP\)?\([.:].*\)".format(alias), "ALIAS", "MY_PV:SP\\2"],
    ...
    def test_GIVEN_remote_PV_with_VAL_WHEN_alias_generated_THEN_lines_as_expected(self):
        ...
        alias = "INST:BLOCK:MY_BLOCK"
        expected_lines = [
            [r"{}\([.:].*\)".format(alias), "ALIAS", "MY_TEST\\1"],
    ...
    def test_GIVEN_local_lowercase_PV_without_suffix_or_SP_WHEN_alias_generated_THEN_lines_as_expected(
        self,
    ):
        blockname, block_pv = "My_Block", "MY_PV"
        alias = "INST:BLOCK:My_Block"
    ...
    def test_GIVEN_configuration_has_pvlist_WHEN_set_new_aliases_THEN_pvlist_copied(
        ...
        copyfile_mock.assert_called_once_with(
            os.path.join(config_dir, "gwblock.pvlist"), self.gateway_file_path
        )
    ...
    def test_GIVEN_configuration_claims_has_pvlist_BUT_does_not_WHEN_set_new_aliases_THEN_pvlist_copied(
        ...
        blocks = {
            expected_name: Block(
                expected_name, expected_pv, log_periodic=True, log_rate=0, log_deadband=1
            )
        }
        alias = "INST:BLOCK:block"
        expected_lines = [
            line.split()
            for line in ALIAS_HEADER.format("INST:").splitlines()
            if not line.startswith("##") and line != ""
        ] + [
